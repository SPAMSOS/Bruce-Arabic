...
void IRMenu::optionsMenu() {
    ...
    options = {
        {"تلفاز",                  StartTvBGone              },
        {"مخصص اشعة تحت الحمراء",  otherIRcodes              },
        {"قراءة اشعة تحت الحمراء", [=]() { IrRead(); }       },
#if !defined(LITE_VERSION)
        {"تشويش اشعة تحت الحمراء", startIrJammer             }, // Simple frequency-adjustable jammer
#endif
        {"إعدادات",                [this]() { configMenu(); }},
    };
    addOptionToMainMenu();

    String txt = "الأشعة تحت الحمراء";
...
void IRMenu::configMenu() {
    options = {
        {"طرف الإرسال", lambdaHelper(gsetIrTxPin, true)},
        {"طرف الاستقبال", lambdaHelper(gsetIrRxPin, true)},
        {"عدد التكرارات", setIrTxRepeats},
