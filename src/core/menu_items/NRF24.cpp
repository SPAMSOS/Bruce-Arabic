...
void NRF24Menu::optionsMenu() {
    options.clear();
    options.push_back({"معلومات", nrf_info});

    if (bruceConfigPins.NRF24_bus.mosi == bruceConfigPins.SDCARD_bus.mosi &&
        bruceConfigPins.NRF24_bus.mosi != GPIO_NUM_NC)
        options.push_back({"الطيف", [=]() { nrf_spectrum(&sdcardSPI); }});
#if TFT_MOSI > 0 // Display doesn't use SPI bus
    else if (bruceConfigPins.NRF24_bus.mosi == (gpio_num_t)TFT_MOSI)
        options.push_back({"الطيف", [=]() { nrf_spectrum(&tft.getSPIinstance()); }});
#endif
    else options.push_back({"الطيف", [=]() { nrf_spectrum(&SPI); }});

    options.push_back({"تشويش FRN", nrf_jammer});

    options.push_back({"تشويش القناة", nrf_channel_jammer});
    options.push_back({"تنقل القنوات", nrf_channel_hopper});

#if defined(ARDUINO_M5STICK_C_PLUS) || defined(ARDUINO_M5STICK_C_PLUS2)
    options.push_back({"إعداد الأطراف", [this]() { configMenu(); }});
#endif
...
void NRF24Menu::configMenu() {
    ...
    options = {
        {"42FRN (قديم)",      [&]() { opt = 1; }         },
        {"42FRN (IPS مشترك)", [&]() { opt = 2; }         },
        {"رجوع",              [this]() { optionsMenu(); }},
    };

    loopOptions(options, MENU_TYPE_SUBMENU, "Config", idx);
    if (opt == 1) {
