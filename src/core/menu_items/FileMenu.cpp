...
void FileMenu::optionsMenu() {
    options.clear();
    if (setupSdCard()) options.push_back({"بطاقة DS", [=]() { loopSD(SD); }});
    options.push_back({"الملفات", [=]() { loopSD(LittleFS); }});
    options.push_back({"واجهه الويب", loopOptionsWebUi});

#if defined(SOC_USB_OTG_SUPPORTED)
    options.push_back({"تخزين جماعي", [=]() { MassStorage(); }});
#endif
    addOptionToMainMenu();

    loopOptions(options, MENU_TYPE_SUBMENU, "Files", 0, true);
}
