...
void loadArabicFont(int size) {
    static int loadedSize = 0;
    if (arabicFontLoaded && loadedSize == size) return;
    if (arabicFontLoaded) tft.unloadFont();
    tft.loadFont("Arabic_" + String(size), LittleFS);
    arabicFontLoaded = true;
    loadedSize = size;
}

void unloadArabicFont() {
    if (arabicFontLoaded) {
        tft.unloadFont();
...
void drawArabicText(const String &text, int x, int y, uint16_t color) {
    loadArabicFont(22);
    String shaped = shapeArabic(text);
    tft.setTextColor(color, TFT_BLACK);
    int textWidth = tft.textWidth(shaped);
    tft.setCursor(x - textWidth, y);
    tft.print(shaped);
}

void drawArabicCenter(const String &text, int y, uint16_t color) {
    loadArabicFont(22);
    String shaped = shapeArabic(text);
    tft.setTextDatum(TC_DATUM);
    tft.drawString(shaped, tft.width() / 2, y);
}

bool isArabicText(const String &text) {
    for (int i = 0; i < text.length() - 1; i++) {
        uint8_t b1 = (uint8_t)text[i];
        uint8_t b2 = (uint8_t)text[i + 1];

        if (b1 >= 0xD8 && b1 <= 0xDB) return true;
    }
    return false;
}
...
void TouchFooter(uint16_t color) {
    ...
    loadArabicFont(22);
    tft.setTextDatum(TC_DATUM);
    tft.drawString(shapeArabic("السابق"), tftWidth / 6, tftHeight + 4);
    tft.drawString(shapeArabic("اختر"), tftWidth / 2, tftHeight + 4);
    tft.drawString(shapeArabic("التالي"), 5 * tftWidth / 6, tftHeight + 4);
    unloadArabicFont();
}
/***************************************************************************************
** Function name: TouchFooter
...
void MegaFooter(uint16_t color) {
    ...
    loadArabicFont(22);
    tft.setTextDatum(TC_DATUM);
    tft.drawString(shapeArabic("خروج"), tftWidth / 6, tftHeight + 4);
    tft.drawString(shapeArabic("أعلى"), tftWidth / 2, tftHeight + 4);
    tft.drawString(shapeArabic("أسفل"), 5 * tftWidth / 6, tftHeight + 4);
    unloadArabicFont();
}
...
void displayRedStripe(String text, uint16_t fgcolor, uint16_t bgcolor) {
    ...
    tft.drawPixel(0, 0, 0);
    tft.fillRoundRect(10, tftHeight / 2 - 18, tftWidth - 20, 36, 7, bgcolor);
    tft.setTextColor(fgcolor, bgcolor);

    if (isArabicText(text)) {
        loadArabicFont(22);
        tft.setTextDatum(TC_DATUM);
        tft.drawString(shapeArabic(text), tftWidth / 2, tftHeight / 2 - 8);
        unloadArabicFont();
    } else {
        if (size == FM) {
            tft.setTextSize(FM);
...
void drawSubmenu(int index, std::vector<Option> &options, const char *title) {
    ...
    String firstOptionStr = index - 1 >= 0 ? options[index - 1].label : options[menuSize - 1].label;
    String selectedStr = options[index].label;
    String thirdOptionStr = index + 1 < menuSize ? options[index + 1].label : options[0].label;

    bool needArabic =
        isArabicText(firstOptionStr) || isArabicText(selectedStr) || isArabicText(thirdOptionStr);
    if (needArabic && !arabicFontLoaded) loadArabicFont(22);

    tft.setTextColor(bruceConfig.secColor);
    tft.fillRect(6, middle_up - 12, tftWidth - 12, 20 * FM, bruceConfig.bgColor);

    if (isArabicText(firstOptionStr)) {
        tft.setTextDatum(TC_DATUM);
        tft.drawString(shapeArabic(firstOptionStr), tftWidth / 2, middle_up);
    } else {
        tft.drawCentreString(firstOptionStr.c_str(), tftWidth / 2, middle_up, SMOOTH_FONT);
    }

    int selectedTextSize = selectedStr.length() <= tftWidth / (LW * FG) - 1 ? FG : FM;
    tft.setTextSize(selectedTextSize);
    tft.setTextColor(bruceConfig.priColor);
    tft.fillRect(6, middle - FG * LH / 2 - 12, tftWidth - 12, FG * LH + 28, bruceConfig.bgColor);

    if (isArabicText(selectedStr)) {
        tft.setTextDatum(TC_DATUM);
        tft.drawString(shapeArabic(selectedStr), tftWidth / 2, middle - selectedTextSize * LH / 2);
    } else {
        tft.drawCentreString(
            selectedStr.c_str(), tftWidth / 2, middle - selectedTextSize * LH / 2, SMOOTH_FONT
        );
    }

    if (isArabicText(selectedStr)) {
        tft.drawFastHLine(
            tftWidth / 4, middle + selectedTextSize * LH / 2 + 10, tftWidth / 2, bruceConfig.priColor
        );
    } else {
        tft.drawFastHLine(
            tftWidth / 2 - selectedStr.length() * selectedTextSize * LW / 2,
            middle + selectedTextSize * LH / 2 + 1,
            selectedStr.length() * selectedTextSize * LW,
            bruceConfig.priColor
        );
    }
    ...
    tft.setTextSize(FM);
    tft.setTextColor(bruceConfig.secColor);
    tft.fillRect(6, middle_down - 12, tftWidth - 12, 20 * FM, bruceConfig.bgColor);

    if (isArabicText(thirdOptionStr)) {
        tft.setTextDatum(TC_DATUM);
        tft.drawString(shapeArabic(thirdOptionStr), tftWidth / 2, middle_down);
    } else {
        tft.drawCentreString(thirdOptionStr.c_str(), tftWidth / 2, middle_down, SMOOTH_FONT);
    }
